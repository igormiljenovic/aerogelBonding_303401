...
<img src="images/BondingStatus.png" alt="Bonding status" />

### Correlation Heatmap

The correlation heatmap is a crucial step in understanding the relationships between features in the dataset and their influence on the target variable, "BondingSuccessful." This visual representation enables us to identify patterns, redundancies, and key predictors. It also guides feature selection and provides a deeper understanding of the dataset’s structure. This section presents an analysis of the correlation heatmap, highlighting its critical role in data preparation and feature engineering.

#### Generating the Correlation Heatmap

The correlation heatmap was generated by calculating the Pearson correlation coefficient for all numerical features in the dataset. This matrix visualized the strength and direction of relationships between features, with values ranging from -1 (perfect negative correlation) to 1 (perfect positive correlation). Features with strong positive or negative correlations with "BondingSuccessful" were flagged for closer examination, as were highly correlated pairs of features indicating potential redundancy.

<img src="images/corrmapInit.png" alt="Correlation heatmap" />

#### Observations from the Heatmap

The correlation heatmap provided several insights into feature relationships. "TotalMaterialProcessed" and "OtherCompaniesMaterialProcessed" exhibited an extremely high correlation (>0.97), indicating redundancy. Retaining both features could introduce multicollinearity, so one of them was considered for exclusion. Similarly, "WorkExperience" and "ApplicantAge" had a high correlation (>0.98), suggesting overlapping information. "SkillRating" displayed a moderately strong positive correlation with "BondingSuccessful," making it a critical predictor, while "ChurnRisk" had a moderate negative correlation with the target variable, indicating that higher churn risk is linked to unsuccessful outcomes.

For successful bondings, "RequestedProcessAmount" positively correlated with "ProcessedKilograms," suggesting that higher processing quantities align with success. In contrast, for unsuccessful bondings, "BondingRiskRating" correlated more strongly with "ChurnRisk," highlighting risk assessment as a significant factor for failure. Weakly correlated features like "MonthlyExecutions" and "ByproductRation" were flagged as potentially irrelevant for prediction and considered for exclusion.

#### Feature Selection and Refinement

Insights derived from the heatmap informed several dataset refinements. Highly correlated features were excluded to reduce redundancy, and key predictors such as "SkillRating" and "ChurnRisk" were prioritized for modeling. Transformations were applied to non-linear relationships, and interactions between features like "BondingRiskRating," "ChurnRisk," and "SkillRating" were explored for potential feature engineering opportunities.

#### Preprocessing for Final Heatmap

Before generating the final heatmap, preprocessing steps were applied to refine the dataset further. Redundant and irrelevant features were removed to enhance interpretability and ensure accurate modeling. Highly correlated features, such as "OtherCompaniesMaterialProcessed" and "WorkExperience," were excluded to reduce multicollinearity. Weakly correlated features, including "MonthlyExecutions" and "ByproductRation," were also removed to streamline the dataset.

Outliers in key numerical variables, such as "ProcessedKilograms" and "RequestedProcessAmount," were addressed using the Interquartile Range (IQR) method. This ensured that extreme values did not distort correlations or impact the model's predictive accuracy. Finally, all numerical features were scaled to standardize their ranges, facilitating a more uniform analysis in the final heatmap.

<img src="images/outliers.png" alt="Outliers" />

#### Final Heatmap

The final heatmap was generated using the cleaned and preprocessed dataset. This visualization highlighted refined relationships between numerical features and the target variable, "BondingSuccessful." The final heatmap revealed several critical insights:

"BondingRiskRating" exhibited a strong negative correlation (-0.85) with "BondingSuccessful," underscoring its importance in predicting unsuccessful outcomes. "ProcessedKilograms" displayed a moderate positive correlation (0.56) with the target variable, indicating its relevance in successful bonding processes. "SkillRating" and "RequestedProcessAmount" also emerged as significant predictors, with moderate correlations to "BondingSuccessful."

The refined dataset and final heatmap provide a solid foundation for subsequent machine learning modeling, ensuring that only the most relevant and informative features are included.

<img src="images/corrMapRefined.png" alt="Correlation Map ALL" />

<img src="images/corrMapRefinedPositive.png" alt="Correlation Map Positive" />

<img src="images/corrMapRefinedNegative.png" alt="Correlation Map Negative" />
